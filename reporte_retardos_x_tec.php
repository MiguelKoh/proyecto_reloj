...
function descuento($idEmp,$idPeriodo,$horaini,$horafin,$idTipoE) {
 ...
 return $diaD;
    ...
    $fechaini = "16/02/2020";     //CAMBIO DE ACUERDO AL PERIODO
    $fechafin = "29/02/2020";     //CAMBIO DE ACUERDO AL PERIODO
...
$pdf->AliasNbPages();
 $pdf->AddPage();
   ...
   $sente = "Select e.idEmp,e.Nombre,d.Nombre as departamento,e.idTipo, t.Descripcion as tipoEmp from empleado e 
   INNER JOIN departamento d ON d.idDepto=e.idDepto 
   INNER JOIN tipoempleado t ON t.idTipo=e.idTipo WHERE e.estatus is null AND e.idTipo=2 ORDER BY e.idTipo, e.Nombre";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        WHERE idEmp=".$row['idEmp']." AND idPeriodo=".$id_periodo."";
        ...
        $cantidad=$Ocupado*15;
                if($cantidad>0 && $cantidad<90){
                    $texto="En la 2da. quincena de junio utilizaste ".$cantidad." de los 90 minutos permitidos al mes.";
                  }else{
                     if($cantidad>0 && $cantidad>=90){
                    $texto="En la 2da. quincena de junio agotaste los 90 minutos permitidos al mes.";
                    }
                  }
    ...
    $tot_min_descuentoF=0;
     ...
     $numr=mysqli_num_rows($result1);
            ...
            while ($row1 = mysqli_fetch_array($result1)) { //sacamos las fechas            	
                ...
                $SQLhorarioWeb="SELECT horario,hora_ini,hora_fin 
                FROM horarios_semestre 
                ...
                $existeRegistro=0;
                $existeRegistro=mysqli_num_rows($queryD);
                if($existeRegistro==0){
                    $sum=$sum+5;
                }else{
                ...
                if($horassf>=$horasf && $horasxf>=$horasff){                 
                       $ausente = "";
                        ...
                        if($idTipoE==2){//administrativos
                                    ...
                                    $minDesS=($enSES-$enSS); 
                                    if($minDesS>0){
                                        $minDescuento= $minDescuento+$minDesS;
                                        
                                    }                              
                                    
                                     //$minDescuento= $minDesS;
                                     //$tot_min_descuento += $minDescuento;
                   ...
                   $pdf->SetFont('Arial','',10);
		            ...
		            $dia = $separar[0];
                    if($minDescuento>0){
                      $tot_min_descuentoF += $minDescuento."<br>";
	...
	//echo $numbaja."<br>";
                ...
                $pdf->Cell(5,5,"",0,0,'C');		
                 ...
                 $tot_min_descuento_formato = convierte_mins_a_horas($tot_min_descuentoF,"N");
			      $pdf->Cell(90,5,'',0,0,'L'); } 
			     $pdf->SetFont('Arial','B',10);
