	...
	class HorizontalChart extends BarChart
	{
		...
		*/
		...
		function printBar()
		{
			...
			///agregamos la suma de todos los valores
			$suma=0;
			for($i = 0; $i < $this->sampleCount; $i++)
			{
				$point = current($this->point);
			...
			for($i = 0; $i < $this->sampleCount; $i++)
			{
				...
				//aqui se imprime el valor de la columna
				$this->text->printText($this->img, $xmax + 5, $y - $rowHeight / 2, $this->textColor,round(100*($value/$suma),2)."% (".$value.")", $this->text->fontCondensed, $this->text->VERTICAL_CENTER_ALIGN);
				//imprimir los totales
				$this->text->printText($this->img, $this->graphTLX,$this->height-10, $this->textColor,"TOTAL = ".$suma, $this->text->fontCondensed, $this->text->VERTICAL_CENTER_ALIGN);
				// Horizontal bar
