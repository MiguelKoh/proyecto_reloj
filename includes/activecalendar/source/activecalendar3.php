...
function activeCalendar3($year=false,$month=false,$day=false,$GMTDiff="none"){
$this->timetoday=time();
$this->selectedday=-2;
...
// checking before enabling, as week number calulation works only if php version > 4.1.0 [php function: date ("W")]
	if (is_integer($this->getWeekNum($this->actday))){
		...
		if ($javaScript) $this->javaScriptWeek=$javaScript;
...
********************************************************************************
*/
function setEvent($year,$month,$day,$id=false,$tooltip=false,$url=false){
...
$this->calEventsToolTip[$eventTime]=$tooltip;
}
/*
********************************************************************************
...
/***********************/
function setPrincipalTitle($title)
{
	$this->myTitle=$title."<br>";
}
/***********************/
/*
----------------------
...
var $calEventsToolTip=false; //variable agregada para los tool tips vector
var $eventToolTip=false; //variable agregada pra un tool tip
/*
********************************************************************************
...
*/
function mkYearHead(){
return "<table class=\"".$this->cssYearTable."\" style='background-color:#FFF;border:thin;width:600px;'>\n";
}
/*
********************************************************************************
...
function mkYearTitle(){
	if ($this->rowCount<1 || $this->rowCount>2) $this->rowCount=4;
	if (!$this->yearNav){
		...
		/*Agregar Titulo*/
		$out.="<td class=\"".$this->cssYearTitle."\">".$this->myTitle.$this->actyear."-".($this->actyear+1)."</td>";
		/*Fin de agregar*/
...
$out="<tr>\n";
	for ($x=1; $x<=2; $x++) {
		$this->activeCalendar3($curyear,$stmonth,false,$this->GMTDiff);
		$out.="<td valign=\"top\">\n".$this->showMonth()."</td>\n";
		if ($x == $TrMaker && $x < 12) {
			$out.="</tr><tr>";
...
function mkMonthTitle(){
	if (!$this->monthNav){
		$out="<tr><td class=\"".$this->cssMonthTitle."\" colspan=\"".$this->monthSpan."\" style='color:#009;font-weight:bold;text-align:center;'>";
...
function mkWeekDays(){
	if ($this->startOnSun){
		...
		if ($this->weekNum) $out.="<td  class=\"".$this->cssWeekNumTitle."\">".$this->weekNumTitle."</td>";
		for ($x=1;$x<=6;$x++) $out.="<td class=\"".$this->cssWeekDay."\" style='color:#009;font-weight:bold;text-align:center'>".$this->getDayName($x)."</td>";
		$out.="<td class=\"".$this->cssWeekDay."\" style='color:#009;font-weight:bold;text-align:center'>".$this->getDayName(0)."</td>";
		$out.="</tr>\n";
...
/*
********************************************************************************
PRIVATE mkDay() -> creates each td tag of the month body AQUI SE CONSTRUYEN LOS CONTENIDOS
********************************************************************************
*/
function mkDay($var){
	//$diasSemana= $this->actyear."-".$this->actmonth."-".$var;
	$diasSemana= $var."-".$this->actmonth."-".$this->actyear;
	$completMes=count($this->actmonth);
	if($completMes==1)
	$fechaReal=str_pad($this->actmonth,2 , "0", STR_PAD_LEFT);
	else
	$fechaReal=$this->actmonth;
	
	$completDia=count($var);
	if($completDia==1)
	$diaReal=str_pad($var,2 , "0", STR_PAD_LEFT);
	else
	$diaReal=$var;
	
	$fechaReal=$this->actyear."-".$fechaReal."-".$diaReal;
if ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday) $hoy="HOY: ";else $hoy="";
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	if ($this->isEvent($var)){
		if ($this->eventUrl){
			$out="<td class=\"".$this->eventID."\" title='".$hoy.$this->eventToolTip."' ><a href=\"".$this->eventUrl."\">".$var."</a>".$eventContent."</td>";
			$this->eventUrl=false;
		}
		elseif (!$this->dayLinks) $out="<td class=\"".$this->eventID."\" title='".$hoy.$this->eventToolTip."' >".$var.$eventContent."</td>";
		else $out="<td class=\"".$this->eventID."\" title='".$hoy.$this->eventToolTip."' >".$linkstr.$eventContent."</td>";
	}
	...
	elseif ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday){
		 $fechaDiaHoy=date("Y-m-d"); // fecha de hoy 19		
		 $semanaProg=date('Y-m-d', strtotime('-1 week'));		
		//if($fechaReal>=$semanaProg && $fechaDiaHoy>=$semanaProg){
			
			$poner="<input type='checkbox' name='fechajust[]' value=".$diasSemana." >";
		//}
		if (!$this->dayLinks) $out="<td class=\"".$this->cssToday."\" title='HOY' style='color:#0000FF;font-weight:bold'>".$poner."<br/>".$var."</td>";
		else $out="<td class=\"".$this->cssToday."\" title='HOY' >".$linkstr."</td>";
	}
	elseif ($this->getWeekday($var)==0 && $this->crSunClass){ //Domingo
	
		if (!$this->dayLinks) $out="<td class=\"".$this->cssSunday."\" style='color:#000;font-weight:bold'><br/>".$var."</td>";
		else $out="<td class=\"".$this->cssSunday."\" >".$linkstr."</td>";
		
		
	}
	elseif ($this->getWeekday($var)==7 && $this->crSatClass){
		 $fechaDiaHoy=date("Y-m-d"); // fecha de hoy 19		
		 $semanaProg=date('Y-m-d', strtotime('-1 week')) ;
		//if($fechaReal>=$semanaProg && $fechaDiaHoy>=$semanaProg){
			...
			$poner="";
		//}
		if (!$this->dayLinks) $out="<td class=\"".$this->cssSaturday."\"><strong>".$poner."</strong><br/><strong>".$var."</strong></td>";
		else $out="<td class=\"".$this->cssSaturday."\" >".$linkstr."</td>";
	}
	else{
		 $fechaDiaHoy=date("Y-m-d"); // fecha de hoy 19		
		 $semanaProg=date('Y-m-d', strtotime('-1 week')) ;//cambiar a uno en lugar de 3
		//if($fechaReal>=$semanaProg && $fechaDiaHoy>=$semanaProg ){
			
			$poner="<input type='checkbox' name='fechajust[]' value=".$diasSemana." >";
	//	}
		if (!$this->dayLinks) $out="<td class=\"".$this->cssMonthDay."\"><strong>".$poner."</strong><br/><strong>".$var."</strong></td>";
		else $out="<td class=\"".$this->cssMonthDay."\">".$linkstr."</td>";
	}
return $out;
}
/*
...
$out="";
	if ($this->weekUrl){
		...
		//$out.="<a href=\"".$this->weekUrl.$glue.$this->yearID."=".$year."&amp;".$this->weekID."=".$week."\" TARGET=\"_new\">".$week."</a>";
		$out.="<a href=\"javascript:".$this->javaScriptWeek."('".$this->weekUrl.$glue.$this->yearID."=".$year."&amp;".$this->weekID."=".$week."')\">".$week."</a>";
	}
	else
		if ($this->javaScriptWeek) 
		{
...
/*
********************************************************************************
PRIVATE isEvent() -> checks if a date was set as an event and creates the eventID (css layout) and eventUrl AQUI ESTA LO QUE BUSCABA
********************************************************************************
*/
	...
	if ($this->calEvents){
		...
		foreach($this->calEvents as $eventTime => $eventID){
			if ($eventTime==$checkTime){
				...
				if ($this->calEventsToolTip[$eventTime]) $this->eventToolTip=$this->calEventsToolTip[$eventTime]; //linea agreagda para los tool tip
				return true;
			}
		}
	return false;
