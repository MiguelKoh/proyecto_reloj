...
function mkYearTitle(){
	...
	if (!$this->yearNav){
		...
		/*Agregar Titulo*/
		$out.="<td class=\"".$this->cssYearTitle."\">"."CALENDARIO UNIVERSITARIO<br>".$this->actyear."-".($this->actyear+1)."</td>";
		/*Fin de agregar*/
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	elseif ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday){
		if (!$this->dayLinks) $out="<td class=\"".$this->cssToday."\" title='HOY' >".$var.$eventContent."</td>";
		else $out="<td class=\"".$this->cssToday."\" title='HOY' >".$linkstr.$eventContent."</td>";
	}
