...
function activeCalendar4($year=false,$month=false,$day=false,$GMTDiff="none"){
$this->timetoday=time();
$this->selectedday=-2;
...
*/
function mkYearHead(){
return "<table class=\"".$this->cssYearTable."\" style='background-color:#EDF9FE;border:thin;width:600px;'>\n";
}
/*
********************************************************************************
...
$out="<tr>\n";
	for ($x=1; $x<=2; $x++) {
		$this->activeCalendar4($curyear,$stmonth,false,$this->GMTDiff);
		$out.="<td valign=\"top\">\n".$this->showMonth()."</td>\n";
		if ($x == $TrMaker && $x < 12) {
			$out.="</tr><tr>";
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	elseif ($this->getWeekday($var)==6 && $this->crSatClass){
		...
		if (!$this->dayLinks) $out="<td class=\"".$this->cssSaturday."\"><strong>".$poner."</strong><br/><strong>".$var."</strong></td>";
		 ...
		 $fechaDiaHoy=date("Y-m-d"); // fecha de hoy 19		
		 $semanaProg=date('Y-m-d', strtotime('-3 week')) ;//cambiar a uno en lugar de 3
		if($fechaReal>=$semanaProg && $fechaDiaHoy>=$semanaProg ){
