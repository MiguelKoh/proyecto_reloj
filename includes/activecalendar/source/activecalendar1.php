...
function activeCalendar1($year=false,$month=false,$day=false,$GMTDiff="none"){
$this->timetoday=time();
$this->selectedday=-2;
...
*/
function mkYearHead(){
return "<table align='center' class=\"".$this->cssYearTable."\" style='background-color:#EDF9FE;border:thin;width:600px;'>\n";
}
/*
********************************************************************************
...
$out="<tr>\n";
	for ($x=1; $x<=2; $x++) {
		$this->activeCalendar1($curyear,$stmonth,false,$this->GMTDiff);
		$out.="<td valign=\"top\">\n".$this->showMonth()."</td>\n";
		if ($x == $TrMaker && $x < 12) {
			$out.="</tr><tr>";
...
function mkWeekDays(){
	if ($this->startOnSun){
		...
		for ($x=0;$x<=6;$x++) $out.="<td class=\"".$this->cssWeekDay."\" style='color:#009;font-weight:bold;text-align:center'>".$this->getDayName($x)."</td>";
		$out.="</tr>\n";
	}
	else{
		$out="<tr>";
		...
		$out.="<td class=\"".$this->cssWeekDay."\" style='color:#FF0000;font-weight:bold;text-align:center'>".$this->getDayName(0)."</td>";
		$out.="</tr>\n";
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	elseif ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday){
		...
		if($fechaReal>=$semanaProg && $fechaDiaHoy>=$semanaProg){
			...
			$poner="<input type='radio' name='fechaUnaJus' value=".$diasSemana." >";
		}
	...
	elseif ($this->getWeekday($var)==0 && $this->crSunClass){ //Domingo
		if (!$this->dayLinks) $out="<td class=\"".$this->cssSunday."\" style='color:#FF0000;font-weight:bold'><br/>".$var."</td>";
	...
	elseif ($this->getWeekday($var)==6 && $this->crSatClass){
		...
		else $out="<td class=\"".$this->cssSaturday."\" ><strong>".$linkstr."</td>";
	}
	else{
			 $fechaDiaHoy=date("Y-m-d"); // fecha de hoy 19		
		 $semanaProg=date('Y-m-d', strtotime('-1 week')) ;
