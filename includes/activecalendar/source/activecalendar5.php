...
function activeCalendar5($year=false,$month=false,$day=false,$GMTDiff="none"){
$this->timetoday=time();
$this->selectedday=-2;
...
$out="<tr>\n";
	for ($x=1; $x<=2; $x++) {
		$this->activeCalendar5($curyear,$stmonth,false,$this->GMTDiff);
		$out.="<td valign=\"top\">\n".$this->showMonth()."</td>\n";
		if ($x == $TrMaker && $x < 12) {
			$out.="</tr><tr>";
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	elseif ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday){
		 ...
		 $semanaProg=date('Y-m-d', strtotime('-3 week'));		
