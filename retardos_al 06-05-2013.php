            ...
            if ($tipoRep == "1"){           
                $sente = "select idemp,nombre,departamento,horario,STR_TO_DATE(fecha,'%d/%m/%Y') as fecha,".
                           ...
                           // "' and TIMEDIFF(horaIni,checadaini1)<0 " .
                            " order by fecha";
							/*$sente ="select idemp,nombre,departamento,horario,STR_TO_DATE(fecha,'%d/%m/%Y') as fecha,horaIni,
horaFin,checadaIni1,checadaFin1,ausente, TIMEDIFF(horaIni,checadaIni1) as difentrada,
TIMEDIFF(horaFin,checadaFin1) as difsalida,TIMEDIFF(checadaFin1,checadaIni1) as horas_totales,
TIMEDIFF(CASE WHEN checadaFin1='' THEN 0 WHEN checadaFin1 THEN horaFin ELSE horaFin END,
CASE WHEN checadaIni1='' THEN 0 WHEN checadaIni1 THEN checadaIni1 ELSE horaIni END) as horas_segun_horario,excepcion,
jornadaTrabajada, attTime as jornadaReal from registros where departamento = 'MANUALES' and idemp ='7875' 
and STR_TO_DATE(fecha,'%d/%m/%Y') >= '2013-04-16' and STR_TO_DATE(fecha,'%d/%m/%Y') <= '2013-04-30' 
and ((TIMEDIFF(horaIni,checadaIni1)<0 or TIMEDIFF(horaFin,checadaFin1)>0 or ausente='Y')) ";*/
							//echo "depto y empleado especifico"."<br>";
            }else{
                if ($tipoRep == "2"){
                    $sente = "select idemp,nombre,departamento,horario,STR_TO_DATE(fecha,'%d/%m/%Y') as fecha,".
                                "horaIni,horaFin,checadaini1,checadafin1," .
