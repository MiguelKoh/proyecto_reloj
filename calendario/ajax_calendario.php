...
switch ($_GET["accion"])
{
	case "guardar_evento":
	{
		 $query="insert into tcalendario (fecha,evento,tipoempleado,horainicial,horafin) values ('".$_GET["fecha"]."','".utf8_decode($_GET["evento"])."','".($_GET["tipo"])."',
		'".($_GET["inicial"])."','".($_GET["final"])."')";
		//mysqli_select_db($cn,$dbname);
		if ($resultado=mysqli_query($cn,$query)) echo "<p class='ok'>Evento guardado correctamente.</p>";
	...
	case "borrar_evento":
	{
		...
		if ($resultado=mysqli_query($cn,$query)) echo "<p class='ok'>Evento eliminado correctamente.</p>";
	...
	case "generar_calendario":
	{
		...
		echo "<table class='calendario' cellspacing='0' cellpadding='0'>";
				...
				if ($tr<$totalfilas)
				{
					if ($i>=$primeromes && $i<=$tope) 
					{
						...
						for($e=0;$e<$total_eventos;$e++)
						{
							...
							{
								$eventos_del_dia.="<p>".utf8_encode($eventos[$e]["evento"])."<a href='#' class='eliminar_evento' rel='".$eventos[$e]["id"]."' title='Eliminar este Evento del ".fecha($fecha_completa)."'><img src='calendario/images/delete.png' /></a></p>";
							}
						}
						if ($eventos_del_dia!="")
						{
						...
						/*if (date("Y-m-d")<=$fecha_completa)*/ echo "<a href='#' data-evento='#nuevo_evento' title='Agregar un Evento el ".fecha($fecha_completa)."' class='modal agregar_evento' rel='".$fecha_completa."'><img src='calendario/images/add.png' /></a>";
			...
			echo "<div id='nuevo_evento' class='window'>";
				...
				<tr>
				 	<td><b>Evento</b></td>
				 	<td> <input type='text' name='evento_titulo' id='evento_titulo'></td>
				 </tr>
				 <tr>
				 	<td><b>Tipo Empleado</b></td>
				 	<td>
					<select name='tipo' id='tipo'>";
                         ...
                         echo "<option value='0'>Seleccione</option>";
                          ...
                          echo "<option value=";
                          echo $rsA['idTipo'];  echo $selected;
                          echo ">";
                              echo utf8_encode($rsA["Descripcion"]);                
                         echo  "</option>";
                         } mysqli_free_result($queryA);
                        echo "</select>	";
				 echo "</td></tr>
				<tr>
				 	<td><b>Horario</b></td>
				 	<td> <input id='inicial' class='inicial' type='text' name='inicial'> a <input id='final' class='final' type='text' name='final'></td>
				 </tr>
				 <tr>
				 	<td><b>Formato 24hrs.</b></td>
				 	<td><b> hh:mm ejemp: 10:15, 14:15 </b> </td>
				 </tr>
				 <tr>
				 	<td></td>
				 	<td><br><br> <input type='button' name='Enviar' value='Guardar' class='enviar'>
				   <input type='hidden' name='evento_fecha' id='evento_fecha'></td>
				 </tr>
