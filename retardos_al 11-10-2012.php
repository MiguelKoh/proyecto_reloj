...
function hayParcial($fechaini,$fechafin){
    ...
    $sente = "select idParcial,fecha_ini,fecha_fin from parciales
                    where fecha_ini >= '" . $fechaini .                         
                    "' and fecha_fin <= '" . $fechafin ."'";        

    //echo "<tr><td>".$sente."</td></tr>";
    
    $result = mysql_query($sente, $cn_b);
    $row_b = mysql_fetch_array($result);

    $idParcial = 0;
    
    if ($row_b){
        $idParcial = $row_b['idParcial'];
    }
    
    $datos[0]= $idParcial;
    $datos[1]= $row_b['fecha_ini'];
    $datos[2]= $row_b['fecha_fin'];
...
/*    $sente="CREATE TABLE `retardos_temp` (
   ...
   */ 
    ...
    $datos_Parcial = hayParcial($fechaini,$fechafin); 
 ...
 //       if (($es_velador == "SI" && $debe_checar == "SI") || ($es_velador == ""))  {                    
            ...
            /*    if ($numero_empleados > 0){
                ...
                */
                ...
                echo '<tr align="right"' . $clase . '>
                            ...
                            <td align="center" bgcolor="#D8D8D8" style="font-size:14px;border: solid #000000; border-width:2px;"><b>Examen</b></td>
                      </tr>';                         
            ...
            if ($idParcial > 0){
                ...
                if (($fecha_reg_nva >= $fecha_ini_parcial1  &&  $fecha_reg_nva <= $fecha_fin_parcial1) && ($row['departamento'] == "PROFESORES" || $tipo_empleado_desc = 1 )){
                    $verifica_salida = "N";
                    $row['difsalida'] = "00:00";
                    $marca_parcial = "P1";                    
                }
                
                $sente_p="select numAula, numSemestre, hora_ini from maestros_parciales
            ...
            if ($row['ausente']== "Y" ){
                ...
                if (($idParcial > 0  &&  $fecha_reg_nva >= $fecha_ini_parcial1  &&  $fecha_reg_nva <= $fecha_fin_parcial1) && ($row['departamento'] == "PROFESORES" || $tipo_empleado_desc = 1)){
                    $ausente = "SI";
                    $total_minutos_ausente = 0;
                    $marca_parcial = "P1";
            ...
            */
            ...
            if (($minutos_x_descontar != 0) || ($tipoRep == "2")){
                ...
                echo '<tr align="right"' . $clase . ' >
                            ...
                            <td align="center" style="font-size:14px;">' . $mins_permiso . '</td>                           
                            <td align="center" style="font-size:14px;">' . $minutos_x_descontar . '</td>
                            <td align="center" style="font-size:14px;">' . $es_retardo_web . '</td>
                            <td align="center" style="font-size:14px;">' . $total_minutos_entrada . '</td>
                            <td align="center" style="font-size:14px;">' . $minutos_x_descontar_e . '</td>    
                            <td align="center" style="font-size:14px;">' . $total_minutos_salida .'</td>
