...
$pdf->AliasNbPages();
 $pdf->AddPage();
   ...
   $sente = "Select e.idEmp,e.Nombre,d.Nombre as departamento,e.idTipo, t.Descripcion as tipoEmp from empleado e INNER JOIN departamento d ON d.idDepto=e.idDepto INNER JOIN tipoempleado t ON t.idTipo=e.idTipo WHERE e.estatus is null AND e.idTipo=1 ORDER BY e.idTipo, e.Nombre";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        WHERE idEmp=".$row['idEmp']." AND idPeriodo=141";
        ...
        $cantidad=$Ocupado*15;
                if($cantidad>0 && $cantidad<90){
                    $texto="En la 1era. quincena de enero utilizaste ".$cantidad." de los 90 minutos permitidos al mes.";
                  }else{
                     if($cantidad>0 && $cantidad>=90){
                    $texto="En la 1era. quincena de enero agotaste los 90 minutos permitidos al mes.";
                    }
                  }
	...
	$sum=0;
	 $sente1 = "SELECT DISTINCT(fecha),horaini,horafin FROM checadas_nuevo WHERE idEmp=" . $row['idEmp']." AND idPeriodo=141  GROUP BY fecha ORDER BY fecha ASC";
